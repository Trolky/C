 ...
 * @brief Zdrojový kód jednoduchého parseru aritmetických výrazů zapsaných v tzv. Reverzní polské notaci (RPN),
 *        který využívá implementovaný zásobník.
 * @version 1.O
 * @date 2020-09-08
 */
...
int evaluate_rpn_expression_3(const char *input, calc_num_type *result) {
    ...
    /* Velice zajímavým řešením je tvorba preprocesorového makra! */
    /* Pokud bude expression == 0, tak uvolní alokovaný zásobník a ukončí funkci s návratovou hodnotou 0. */
    /* expression bude při preprocessingu nahrazen konkrétním výrazem, takže zde jsou závorky nezbytné! */
    /* Znakem '\' ukončuji řádky makra. */
...
int main() {
    ...
    for (;;) {
        ...
        if (evaluate_rpn_expression_3(input, &result)) {
            printf("%d\n", result);
        }
