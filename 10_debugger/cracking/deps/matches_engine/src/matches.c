...
static int matches_pc_move(int current_matches) {
...
static int matches_player_move() {
...
void run_matches() {
    ...
    while (matches_count > 0) {
        ...
        if (human_current_player) {
            matches_count -= matches_player_move();
        }
        else {
            matches_count -= matches_pc_move(matches_count);
        }
