...
/** \brief Maximální hodnota, kterou je možné typem `size_t` reprezentovat. */
static const size_t SIZE_T_MAX = ~(size_t)0;
...
/** \brief Pole řazených čísel bude alokováno dynamicky na haldě. */
 ...
 */
int test_ascendness(const int *array, const size_t array_size) {
    size_t i;
...
*/
  #ifdef BUILD_TESTS 
    printf("\nVysledek testu: %s!\n", test_ascendness(numbers, NUMBER_COUNT) ? "OK" : "CHYBA");
  #endif
