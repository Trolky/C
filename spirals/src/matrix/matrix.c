...
int matrix_init(struct matrix *mat, const size_t rows, const size_t cols, const mat_num_type i_val) {
    ...
    mat->items = malloc(rows * cols * sizeof(mat_num_type));
    if (!mat->items) {
        return 0;
    }
...
void matrix_deinit(struct matrix *mat) {
    ...
    /* Možná někdy v budoucnu dojdeme k implementaci, kde matice bez dat bude validní. */
    if (mat->items) {
        free(mat->items);
        mat->items = NULL;
    }
...
void matrix_free(struct matrix **mat) {
    ...
    /* Uvolníme všechno vnitřní členy instance struktury `matrix`. */
    matrix_deinit(*mat);

    /* Uvolníme samotnou dynamicky alokovanou instanci struktury. */
    free(*mat);
    *mat = NULL;
}
