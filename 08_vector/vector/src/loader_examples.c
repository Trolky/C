...
void dynamic_person_loader_example() {
     ...
     * kompilátor začne okamžitě chrlit varování. Když pomineme exotické platformy a zamyslíme se nad mnohem častějším
     * jevem, vlastní blbostí, pak zjistíme, že explicitní konverzí můžeme převést i zcela nesouvisející funkce, třeba
     *
     ...
     *
     * ! Mnohem lepším nápadem je napsat obalovací funkci (třeba `person_deinit_wrapper` definovanou níže), která již bude
     * ! mít správný prototyp. V jejím těle pak provedeme implicitní konverzi samotného ukazatele, což je operace
     * ! definovaná standardem, tzn. jsme v pohodě a můžeme klidně spát (to ostatně děláme při každém volání funkce `malloc`).
     *
     * Zajímavostí ovšem je, že na této explicitní konverzi ukazatelů na funkce (tedy standardem zcela nedefinované
     * operaci) závisí kromě našeho skromného prográmku i nemalé množství v produkci používaných knihoven. Dokonce
     * i samotná GLib! S tou nedefinovaností to tedy nebude tak horké... ale je dobré vědět, že dle standardu je tato
     * konverze nedefinovaná.
     *
     * POZOR! Toto řešení bude fungovat jenom s kompilátorem jazyka C! Ukazatel na funkci v očích C++ je úplně jiná liga.
    */
    ...
    for (size_t i = 0; i < vector_count(persons); ++i) {
        person_print(*(struct person **)vector_at(persons, i));
    }
  #endif
...
void static_person_loader_example() {
    ...
    for (size_t i = 0; i < vector_count(persons); ++i) {
        person_print(vector_at(persons, i));
    }
  #endif
